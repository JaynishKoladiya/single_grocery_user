...
class _PaymentoptionState extends State<Paymentoption> {
  ...
  placeorderAPI(type) async {
    try {
      ...
      print(map);
      var response = await Dio().post(DefaultApi.appUrl + PostAPI.Order, data: map,options: Options(
      ...
      if (placedorederdata!.status == 1) {
        Navigator.of(context).push(MaterialPageRoute(builder: (context) => Ordersucesspage()));
      } else {
            ...
            if (snapshot.hasData) {
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  if(paymentlist!.paymentmethods![index].paymentName == "COD")
                    {
                      return SizedBox();
                    }
                  else
                    {
                      return Column(
                        children: [
                          InkWell(
                            onTap: () {
                              print(paymentlist!.paymentmethods![index].paymentName);
                  ...
                  if (isopendata.isCartEmpty == "0") {
                    if (namepay == "COD") {
                      ...
                      if (double.parse(widget.ordertotal!) >=
                          ...
                          // Navigator.push(
                          //   context,
                          //   MaterialPageRoute(builder: (context) =>  Addmoney(widget.ordertotal)),
                          // );
                        loader.showErroDialog(
